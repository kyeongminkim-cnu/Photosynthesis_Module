...
##########################################################
theta   = 0.7       # hyperbola parameter
width   = 0.1        # leaf width (m)
Vcm25   = 152.5     # Vcmax of CC at 25 C
Jm25    = 238.6      # Jmax  of CC at 25 C
TPU25   = 17.1
Rd25    = 1.7
gamma25 = 42.8        # CO2 compensation point in the absence of mitochondirial respiration
Havcm   = 65330     # Ha
#Hdvcm   = 149252.    # Hd
#Svvcm   = 486.       # Sv
Hajm    = 57500     # Ha
...
class GasExchange():
    ...
    def setValue(self, PPFD, Ta, RH, wind, crop_st, CO2=400):
        ## Input variables
...
#         return Tleaf, Ci, An, Wc, Wj, Ws, Rd, gs, newTleaf, E, G
    ...
    def minh(self, A, B, theta):           # hyperbolic minimum
        ...
        if ( x < 0) : res = min(A,B)
        else: res = ((A+B) - np.sqrt(x)) / (2 * theta)
        return res
    ...
    def EnergyBal(self, An, Tleaf):
        ...
        Ta  = self.Ta
        gbw = self.gbw()
        gsw = self.gsc(An, Tleaf, st)[0] * 1.6            # co2 - > h2o
        gh  = gbw * 0.135 / 0.147
        gv  = gsw * gbw /(gsw + gbw)
    ...
    def gsc(self, An, Tleaf, st):    # H2O  co2 = gsw / 1.6
        gbc = self.gbc()
        Ca = self.Ca
        Ha = self.RH / 100
        ...
        if An <= 0 : An = 0.1
        ah = (st*g1 * An) / Cs
        bh = g0 + gbc - (st*g1 * An / Cs)
        ch = (-Ha * gbc) - g0
        hs = min(max((-bh + np.sqrt(bh*bh - 4*ah*ch)/(2*ah)), 0.1), 1.0)
        gsc = min(g0 + st*g1 * An * hs / Cs, 1.5)
    ...
    def tempParam2(self, k25, Ha, temp):
        return  k25 * np.exp((Ha / (R * (25.+KELVIN)) * (1 - (25.+KELVIN) / (KELVIN + temp))))
